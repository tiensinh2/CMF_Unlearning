...
def _reset_torchvision_vit_head_to_identity(vit: nn.Module):
    """
    your torchvision-style ViT in head in vit.heads (nn.Sequential)
    CMF/remove_FC time：form Identity， forward  logits。
    """
...
class ModelModule(pl.LightningModule):
    def __init__(self, args):
        ...
        if not hasattr(args, "pretrained"): args.pretrained = False
        ...
        self.is_vit = ("vit" in arch)
        self.is_resnet = ("resnet" in arch)
        ...
        if self.is_resnet:
            if arch == "resnet18":
                ...
                full_model = ResNet50(num_classes=args.num_classes, dataset=args.dataset)
                feat_dim = 2048
            else:
                raise ValueError(f"Unsupported ResNet arch: {args.arch}")
        
            if args.remove_FC:
                full_model.fc = nn.Identity()
                self.encoder = full_model  # keeporiginalconstructandlayer name
                self.flatten = nn.Flatten()
                self.feature_dim = feat_dim  # from ResNet config
            else:
            ...
            if arch == "vit_b_16":
                self.encoder = vit_b_16(pretrained=args.pretrained)
                self.feature_dim = 768
            elif arch == "vit_b_32":
                self.encoder = vit_b_32(pretrained=args.pretrained)
                self.feature_dim = 768
            elif arch == "vit_l_16":
                self.encoder = vit_l_16(pretrained=args.pretrained)
                self.feature_dim = 1024
            elif arch == "vit_l_32":
                self.encoder = vit_l_32(pretrained=args.pretrained)
                self.feature_dim = 1024
            elif arch == "vit_h_14":
                self.encoder = vit_h_14(pretrained=args.pretrained)
                self.feature_dim = 1280

            # ---- timm ViT tiny/small ----
            elif ("vit_t_16" in arch) or ("vit_tiny" in arch):
            ...
            elif ("vit_s_16" in arch) or ("vit_small" in arch):
                vit = timm.create_model(
            ...
            if args.remove_FC:
                # timm: reset_classifier(0)
                if hasattr(vit, "reset_classifier"):
                    vit.reset_classifier(0)
                # torchvision-style: heads -> Identity
                else:
                    vit = _reset_torchvision_vit_head_to_identity(vit)

                self.feature_dim = feat_dim
            
            else:
                # not remove_FC time，your forward willoutput logits，not CMF
                self.feature_dim = args.num_classes
        ...
        print(f"[ModelModule] arch={args.arch}, is_vit={self.is_vit}, remove_FC={args.remove_FC}, feature_dim={self.feature_dim}")
    ...
    def extract_features(self, x):
        if getattr(self, "is_vit", False) is False:
            x = self.encoder(x)
            if x.ndim == 4:
                x = torch.flatten(x, start_dim=1)
            return x
        
        # ViT separate： CLS（pre-head）
        vit = self.encoder  # vit backbone
        # torchvision-style vit（youactually/real）have _process_input / class_token / encoder
        if hasattr(vit, "_process_input") and hasattr(vit, "class_token") and hasattr(vit, "encoder"):
            z = vit._process_input(x)              # (B, S, D)
            B = z.shape[0]
            cls = vit.class_token.expand(B, -1, -1)
            z = torch.cat([cls, z], dim=1)         # (B, S+1, D)
            z = vit.encoder(z)                     # (B, S+1, D), encoder cancan LN
            # timm havetimehave vit.norm（encoder after LN），torchvision-style have
            if hasattr(vit, "norm") and vit.norm is not None:
                z = vit.norm(z)
            feat = z[:, 0]                         # (B, D)
            return feat

        # timm-style vit（type）：forward_features Return (B, D) or (B, tokens, D)
        ...
        raise ValueError("Unsupported ViT backbone: cannot extract features.")
