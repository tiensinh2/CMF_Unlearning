...
from models.vit import vit_b_16, vit_b_32,  vit_l_16, vit_l_32, vit_h_14, vit_t_16, vit_s_16, ViT_B_16_Weights, ViT_B_32_Weights,ViT_L_16_Weights, ViT_L_32_Weights, ViT_H_14_Weights
...
def load_encoder_ckpt_safely(model, ckpt_path, device="cpu"):
    """
    model: your CMF ModelModule（have model.encoder）
    ckpt_path: ./checkpoints/pre_train/xxx.pt
    """
    ...
    # 1) cancanis ModelModule keepmaintain：key  "encoder.xxx"
    # 2) cancanis LogitsToLogProb keepmaintain：key  "backbone.xxx"
    # 3) cancanismorelayer：for example "model.encoder.xxx"（ifyoubeforethismaintainpass）
    candidates = [
        sd,
        _strip_prefix_if_present(sd, "model."),
        _strip_prefix_if_present(sd, "encoder."),
        _strip_prefix_if_present(sd, "backbone."),
        _strip_prefix_if_present(sd, "module."),  # DDP 
    ]
    ...
    for s in candidates:
        ret = model.encoder.load_state_dict(s, strict=False)
        m = len(ret.missing_keys)
        u = len(ret.unexpected_keys)
        # choose missing+unexpected minimum's/ofthat time
        if (m + u) < (best_missing + best_unexp):
            best = (s, ret)
            best_missing, best_unexp = m, u

    # re-use best 's/ofagain load once，ensurestateis best
    #print("Wrap:", best[0])
    model.encoder.load_state_dict(best[0], strict=False)
    ret = best[1]

    print(f"[best load] missing={len(ret.missing_keys)} unexpected={len(ret.unexpected_keys)}")
    if len(ret.unexpected_keys) > 0:
        print("unexpected head:", ret.unexpected_keys[:20])
    if len(ret.missing_keys) > 0:
        print("missing head:", ret.missing_keys[:20])

    return ret


def cifar100_get_forget_classes_by_coarse_label(
    root_dir="./data/cifar-100-python",
    forget_coarse_labels=None
):
    """
    Loads CIFAR-100 metadata and training labels, then returns two sorted lists:
    (1) A list of fine-class IDs that belong to the specified coarse labels (forget_classes),
    (2) A list of all other fine-class IDs (retain_classes).

    Args:
        root_dir (str): Path to the 'cifar-100-python' directory containing
                        'meta' and 'train' pickle files.
        forget_coarse_labels (list or set): Coarse label IDs to forget.
                                            Each ID should be in the range [0..19].
                                            If None or empty, no classes are forgotten.

    Returns:
        forget_classes (list[int]): A sorted list of fine-class IDs under the 'forget' coarse labels.
        retain_classes (list[int]): A sorted list of the remaining fine-class IDs (0..99).
    """
    if forget_coarse_labels is None:
        forget_coarse_labels = []

    # 1. File paths
    meta_path  = os.path.join(root_dir, "meta")
    train_path = os.path.join(root_dir, "train")

    # 2. Load meta file (contains label names) -- not strictly needed unless you want names
    with open(meta_path, 'rb') as f:
        meta = pickle.load(f, encoding='latin1')
    # coarse_label_names = meta['coarse_label_names']  # if you need them
    # fine_label_names   = meta['fine_label_names']    # if you need them

    # 3. Load train file (contains label indices for each sample)
    with open(train_path, 'rb') as f:
        train_data = pickle.load(f, encoding='latin1')

    fine_labels   = train_data['fine_labels']   # e.g., [37, 2, 53, ...] length=50000
    coarse_labels = train_data['coarse_labels'] # e.g., [5, 0, 14, ...] length=50000

    # 4. Build {coarse_label -> set_of_fine_labels}
    coarse2fine = defaultdict(set)
    for f_lbl, c_lbl in zip(fine_labels, coarse_labels):
        coarse2fine[c_lbl].add(f_lbl)

    # 5. Collect all forget fine labels from the specified coarse labels
    forget_set = set()
    for c_lbl in forget_coarse_labels:
        forget_set |= coarse2fine[c_lbl]  # union of sets

    # 6. Build the list of all possible fine classes (0..99)
    all_fine_classes = set(range(100))

    # 7. The retain set is all fine classes that are NOT in forget_set
    retain_set = all_fine_classes - forget_set

    # 8. Return two sorted lists
    forget_classes = sorted(forget_set)
    retain_classes = sorted(retain_set)

    return forget_classes, retain_classes



np.set_printoptions(suppress=True)

class LogitsToLogProb_old(nn.Module):
    ...
    def _process_input(self, x):
        return self.backbone._process_input(x)

    @property
    def class_token(self):
        return self.backbone.class_token
...
class LogitsToLogProb(nn.Module):
    ...
    def unwrap_backbone(self) -> nn.Module:
        m = self
        while isinstance(m, LogitsToLogProb):
            m = m.backbone
        return m

    @torch.no_grad()
    def get_head_input(self, x: torch.Tensor) -> torch.Tensor:
        """
        Returnclassification headoutputinputfeature（ViT only）:
        - timm ViT: forward_features -> [B,D] or [B,T,D] get CLS
        - torchvision-style ViT: _process_input + encoder (+norm) -> CLS
        """
        m = self.unwrap_backbone()

        # (A) timm ViT
        if hasattr(m, "forward_features"):
            feat = m.forward_features(x)
            if feat.ndim == 3:      # [B, T, D]
                feat = feat[:, 0]   # CLS
            return feat

        # (B) torchvision-style ViT (you)
        if hasattr(m, "_process_input") and hasattr(m, "encoder") and hasattr(m, "class_token"):
            z = m._process_input(x)         # [B, seq, D]
            B = z.shape[0]
            cls = m.class_token.expand(B, -1, -1)
            z = torch.cat([cls, z], dim=1)  # [B, seq+1, D]
            z = m.encoder(z)                # your encoder cancan ln
            if hasattr(m, "norm") and getattr(m, "norm") is not None:
                z = m.norm(z)
            return z[:, 0]

        raise AttributeError(
            f"get_head_input(): unsupported ViT backbone type: {type(m)}. "
            "Expected timm ViT(with forward_features) or torchvision-style ViT(with _process_input/encoder/class_token)."
        )

    # ---- you LP ：putthissomeproperty/methodto backbone ----
    def _process_input(self, x):
        return self.unwrap_backbone()._process_input(x)

    @property
    def class_token(self):
        return self.unwrap_backbone().class_token

    @property
    def encoder(self):
        return self.unwrap_backbone().encoder

    @property
    def norm(self):
        return getattr(self.unwrap_backbone(), "norm", None)
...
def get_dataset(args):
    ...
    if "fmnist" in args.dataset:
        ...
        if is_vit:
            # === give ViT use's/of CIFAR transform：Resize to 224 + ImageNet mean/std ===
            train_transform = transforms.Compose([
                transforms.Resize(256),            # firstto
                transforms.RandomCrop(224),        # againrandomform 224x224
    ...
    elif "tinyimagenet" in args.dataset.lower() or "tiny-imagenet" in args.dataset.lower():
        tiny_root = os.path.join(args.data_path, "tiny-imagenet-200")
        train_dir = os.path.join(tiny_root, "train")
        val_dir   = os.path.join(tiny_root, "val_sorted")
        ...
        if args.train_transform:
            dataset1 = datasets.ImageFolder(train_dir, transform=train_transform)
        else:
            dataset1 = datasets.ImageFolder(train_dir, transform=test_transform)
        dataset2 = datasets.ImageFolder(val_dir, transform=test_transform)

        args.num_classes = 200

        # get wnid → single 's/of
        wnid_to_words = {}
        with open(os.path.join(tiny_root, "words.txt"), "r") as f:
            for line in f:
                wnid, words = line.strip().split('\t')
                wnid_to_words[wnid] = words.split(',')[0]  # onlygetonename

        # by ImageFolder 's/ofclass（folder name）generateform label name
        # dataset1.classes isonebycharactergood's/ofonedimension list，for example ["n01443537", "n01629819", ...]
        args.class_label_names = [wnid_to_words[wnid] for wnid in dataset1.classes]
...
def reset_vit_head(model, num_classes: int):
    """
    put VisionTransformer 's/ofclassification head 1000 form num_classes。
    adapt Tiny-ImageNet (200 class)、Imagenette (10 class)、 CIFAR (10/100)。
    """
    # your VisionTransformer fixedin，head ismaintaininin model.heads this Sequential in's/of
    if hasattr(model, "heads"):
        # type：model.heads["head"] is nn.Linear
        if isinstance(model.heads, nn.Sequential) and "head" in model.heads._modules:
            in_dim = model.heads.head.in_features
            model.heads.head = nn.Linear(in_dim, num_classes)
        # propertyonedot：havesome heads cancandirectlyis Linear
        elif isinstance(model.heads, nn.Linear):
            in_dim = model.heads.in_features
            model.heads = nn.Linear(in_dim, num_classes)
        else:
            raise ValueError(f"Unexpected ViT heads structure: {model.heads}")
    else:
        raise ValueError("Model has no 'heads' attribute; please check VisionTransformer implementation.")


def reset_timm_vit_head(model, num_classes: int):
    """
    put timm 's/of ViT / DeiT etcmodel's/ofclassification headform num_classes。
    onlylogic's/of head name：
      - model.head
      - model.head.fc
      - model.fc
    """
    if hasattr(model, "head") and isinstance(model.head, nn.Linear):
        ...
        raise ValueError(f"Cannot find classification head in timm model: {model}")


def get_init_model(args,device):
    # Instantiate model
    ...
    #    model = vit_h_14().to(device)
        ...
        if getattr(args, "pretrained", False):
            # use ImageNet-1K trainingmodel
            model = vit_b_16(weights=ViT_B_16_Weights.IMAGENET1K_V1)
            # put 1000 class's/of head formyoubeforedataset's/of num_classes
            reset_vit_head(model, args.num_classes)
        else:
            # randominitialify，directlyfixed num_classes
            model = vit_b_16()
        ...
        model = model.to(device)
    # ======== ViT-Tiny (timm) ========
    elif "vit_t_16" in args.arch.lower() or "vit_tiny" in args.arch.lower():
        if getattr(args, "pretrained", False):
        ...
        model = LogitsToLogProb(backbone).to(device)

    elif "vit_s_16" in args.arch.lower() or "vit_small" in args.arch.lower():
        if getattr(args, "pretrained", False):
            backbone = timm.create_model(
...
def to_jsonable(o):
    ...
    # numpy 
    if isinstance(o, np.generic):   # numpy 
        v = o.item()
        return None if (isinstance(v, float) and (math.isnan(v) or math.isinf(v))) else v
    if isinstance(o, np.ndarray):   # numbergroup -> list
        return o.tolist()

    # torch 
    if _HAS_TORCH and isinstance(o, getattr(torch, "Tensor", ())):
        if o.numel() == 1:
            v = o.detach().cpu().item()
            return None if (isinstance(v, float) and (math.isnan(v) or math.isinf(v))) else v
        return o.detach().cpu().tolist()
    if _HAS_TORCH and isinstance(o, (getattr(torch, "device", ()), getattr(torch, "dtype", ()))):  # type: ignore[arg-type]
        return str(o)
