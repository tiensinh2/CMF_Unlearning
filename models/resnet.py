...
class Bottleneck(nn.Module):
    ...
    def get_scaled_projections(self, x, block_key, alpha, max_samples=10000):
        ...
        proj.update(layer_acts)        
        out = F.relu(self.bn2(out) )
        out, layer_acts = forward_cache_projections(out, self.conv3, f"{block_key}.conv3", max_samples)
        proj.update(layer_acts)   
        out = self.bn3(out)
        for layer in self.downsample: 
...
class ResNet_cifar(nn.Module):
    ...
    def project_weights(self, projection_mat_dict, proj_classifier=False, freeze_except_last=False):
        block_ind=0
        fc_ind = 0
        ...
        self.conv1.weight.data = torch.mm(self.conv1.weight.data.flatten(1), projection_mat_dict["conv1"].transpose(0,1)).view_as(self.conv1.weight.data, freeze_except_last=False)
...
def ResNet18(num_classes=1000, dataset = "imagenet", do_log_softmax=True):
    if "tinyimagenet" in dataset.lower() or "tiny-imagenet" in dataset.lower():
        ResNet = ResNet_cifar
    elif "imagenet" in dataset.lower()  or  "vggface" in dataset.lower():
